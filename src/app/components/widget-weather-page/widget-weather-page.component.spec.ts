...
describe('WidgetWeatherPageComponent', () => {
  ...
  beforeEach(() => {
    const weatherServiceMock = {
      getCityCoordinates: jasmine.createSpy('getCityCoordinates').and.returnValue(of({ lat: 123, lon: 456 })),
      getCityWeather: jasmine.createSpy('getCityWeather').and.returnValue(of(cityWeatherMock)),
    };

    const citiesServiceMock = {
      getCapitalCities: jasmine.createSpy('getCapitalCities').and.returnValue(of([defaultMockCity])),
    };
  ...
  it('Testing getCapitalCities method', (done: DoneFn) => {
    citiesService
      ...
      .subscribe({
        next: (cities) => {
          expect(cities).withContext('expected capital cities').toEqual([defaultMockCity]);
          done();
        },
        error: done.fail,
      });
